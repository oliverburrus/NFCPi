...
while x == 0:
    ...
    filename = '/home/pi/NFCPi/audio/'+ str(datetime.now().strftime('%Y%m%d%H%M%S'))+'.wav'
    os.system('arecord --format=S16_LE --duration=9 --rate=22050 '+str(filename))
    ...
    if net == "NFC":
        ...
        try:
            df1 = pd.DataFrame()
            bn_list = analyze_birdnet(filename, 43.9, -90.0)
